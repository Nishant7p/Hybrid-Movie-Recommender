...
# Reverse lookup: title → item_id (avoids O(n) scan on every movie select)
title_to_item_id: dict[str, int] = {
    meta.get("title", ""): iid for iid, meta in items_lookup.items() if meta.get("title")
}
...
def on_movie_select(user_id: int, movie_title: str) -> str:
    try:
        if not movie_title or user_id is None:
            return ""
        user_id = int(user_id)
        item_id = title_to_item_id.get(movie_title)
        if item_id is None:
        ...
        return f"<p style='color:#ff6e6e'>Error loading prediction: {e}</p>"
